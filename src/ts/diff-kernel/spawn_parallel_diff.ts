...
import { d_path_element } from "./spawn_parallel_worker";
import { FunctionThread, Pool, spawn } from "threads";
...
async function find_d_path(old_string: string, new_string: string, threads: number) : Promise<[number, d_path_element[][]]> {
    const pool = Pool<FunctionThread<[string,string,d_path_element], d_path_element[]>>(() => spawn(new Worker("./spawn_parallel_worker")), threads);
    let d: number = 1;
    ...
    for (;d < old_string.length + new_string.length && !done; ++d) {
        let tasks: QueuedTask<FunctionThread<[string, string, d_path_element], d_path_element[]>,any>[] =
            new Array<QueuedTask<FunctionThread<[string,string,d_path_element],d_path_element[]>, any>>(d);
        ...
        paths[d-1].forEach((path, index) => {
            tasks[index] = pool.queue(forker => fork_d_path(old_string, new_string, path))});
